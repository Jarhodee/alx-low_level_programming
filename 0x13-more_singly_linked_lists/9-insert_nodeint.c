...
listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n)
{
	...
	while (idx > 1 && traverse_xom != NULL)
	{
		idx--;
		traverse_xom = traverse_xom->next;
	}
	if (idx > 1)
	...
	node_to_insert->next = traverse_xom->next;
	/*update the link of prev. node  to be new node's addr. i.e the head*/
	traverse_xom->next = node_to_insert;

	if (traverse_xom == NULL)/*this means node becomes the tail*/
		*head = node_to_insert;
