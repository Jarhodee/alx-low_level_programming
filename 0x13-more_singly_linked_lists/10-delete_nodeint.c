...
int delete_nodeint_at_index(listint_t **head, unsigned int index)
{
	...
	listint_t *node_del = *head;/*the node to delete*/
	...
	if (index == 0)
	{
		...
		*head = node_del->next;/*head points to next node*/
		free(node_del);/*free current head, (as node to del)*/
		node_del = NULL;/*set delete node to be NULL*/
		return (1);
	}
	/*if index is not 0 then traverse to the index*/
	while (index > 0 && node_del != NULL)
	{
		/*set previous node to be the node to del*/
		prev_node = node_del;
		node_del = node_del->next;/*move node to del to idx*/
	...
	/*if node_del == NULL, nothing to delete, return -1*/
	if (node_del == NULL)
		return (-1);
